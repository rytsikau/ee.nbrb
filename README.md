# ee.Nbrb

Данное 1С-приложение предназначено для отображения курсов обмена валют, установленных Национальным банком Беларуси. Доступна информация по всем валютам, начиная с 1991 года. Информация берется с сервера Национального банка.

Возможности программы:

* получение курсов по всем валютам на сегодня
* получение курсов по всем валютам на выбранную дату
* получение курсов по выбранной валюте за произвольную дату или период
* сохранение результата в текстовый или Excel файл

## Системные требования:
* платформа 1С 8.3
* доступ к Интернету

<br>

## Скачать

# [>> управляемая форма <<](https://github.com/rytsikau/ee.nbrb/releases/download/v.2026.01.31/nbrb_upr.epf)
# [>> обычная форма <<](https://github.com/rytsikau/ee.nbrb/releases/download/v.2026.01.11/nbrb_v.20260111.epf)

<br>

## Скриншот (управляемая форма)

<img src='https://github.com/rytsikau/ee.nbrb/blob/200651b52343987ceb2aa3f947c0c33cd36542c6/screenshot_upr.jpg'>

<br>

## Скриншот (обычная форма)

<img src='https://github.com/rytsikau/ee.nbrb/blob/b5ec72d76f3180db2edbfc41f0bd0ef47dfd3c65/screenshot.jpg'>

<br>

## Теги

1C 1С exchange rates archive National Bank of Belarus внешняя обработка курсы валют Нацбанк РБ НБРБ

---
[[start page]](https://rytsikau.github.io) [[author e-mail]](mailto:y.rytsikau@gmail.com)
